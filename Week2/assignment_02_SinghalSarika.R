...
## Use `setwd()` if needed
setwd("~/Users/dragon/Documents/dsc520")

## Load the file `data/tidynomicon/person.csv` to `person_df1` using `read.csv`
## Examine the structure of `person_df1` using `str()`
person_df1 <- read.csv(file = '~/Documents/repo/Week2/dsc520/data/tidynomicon/person.csv')
str(person_df1)
...
person_df2 <- read.csv(file = '~/Documents/repo/Week2/dsc520/data/tidynomicon/person.csv', stringsAsFactors = FALSE)
str(person_df2)
## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- read.csv(file = '~/Documents/repo/Week2/dsc520/data/scores.csv')
summary(scores_df)
...
excel_sheets('~/Documents/repo/Week2/dsc520/data/G04ResultsDetail2004-11-02.xls')
...
voter_turnout_df2 <- read_excel("G04ResultsDetail2004-11-02.xls", sheet= "Voter Turnout", skip=2)
str(voter_turnout_df2)

colnames(voter_turnout_df2) <- c('C1_ward_precint','C2_ballots_cast','C3_registered_voters','C4_voter_turnout')
str(voter_turnout_df2)
...
table_names <- lapply(table_names)
dbReadTable(db, 'table_names')
